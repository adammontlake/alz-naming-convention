...
variable "type" {
  ...
  validation {
    condition = contains([
      "resource_group", "express_route", "managed_identity", "virtual_network", "vpn_gateway", "managed_disk", "ssh_key",
      "subnet_network", "network_interface", "public_ip_network", "express_route_circuit", "express_route_peering",
      "network_security_group", "network_security_group_rule", "application_security_group", "route_table", "key_vault",
      "private_endpoint", "storage_account", "vnet_peering", "vm", "vmss", "firewall", "firewall_Policy", "key_vault_key",
      "firewall_policy_rule_collection_group", "express_route_peering_connection", "firewall_policy_rule_collection",
      "firewall_policy_rule_DNAT", "firewall_policy_rule_app", "dns_private_resolver", "dns_private_resolver_endpoint",
      "firewall_policy_rule_network", "dns_private_resolver_rule_set", "dns_private_resolver_rule_set_rule", "ampls",
      "disk_encryption_set", "key_vault_secret", "vpn_gateway_s2s_connection", "vpn_gateway_er_connection",
      "log_analytics_cluster", "log_analytics_workspace", "private_endpoint_resource", "azure_dashboard", "express_route_connection"
    ], var.type)
...
variable "traffic_protocol" {
  ...
  type        = string
  validation {
    condition     = contains(["tcp", "udp", "icmp", "udp_tcp", "tcp_udp"], var.traffic_protocol)
...
variable "action" {
  ...
  type        = string
  validation {
    condition     = contains(["a", "d"], var.action)
...
variable "law_purpose" {
  description = "Specifies the log analytics purpose."
  type        = string
  validation {
    condition     = var.law_purpose == "sentinel" || var.law_purpose == "diagnostics"
    error_message = "The la_purpose value must be one of sentinel or diagnostics."
  }
  default = "sentinel"
}

variable "vnet_name" {
  description = "(private_endpoint_resource) Specifies the name of the virtual network that the private endpoint resource will be associated to."
  type        = string
  default     = ""
...
variable "resource_name" {
  description = "(private_endpoint_resource) Specifies the name of the target resource that the private endpoint resource will be associated to."
  type        = string
  default     = ""
}

variable "erp_type" {
  description = "Specifies the express route peering type."
  type        = string
  validation {
    condition     = contains(["AzurePrivatePeering", "AzurePublicPeering", "MicrosoftPeering"], var.erp_type)
    error_message = "The erp_type value must be one of AzurePrivatePeering, AzurePublicPeering or MicrosoftPeering."
  }
  default = "AzurePrivatePeering"
}

variable "er_circuit_name" {
  description = "Specifies the express route circuit name."
  type        = string
  default     = ""
}

variable "erp_name" {
  description = "Specifies the express route peering name."
  type        = string
  default     = ""
...
variable "er_gateway_name" {
  description = "Specifies the express route gateway name."
  type        = string
  default     = ""
}
