locals {
  ...
  name_functions = {
    ...
    express_route                         = local.express_route_name
    express_route_circuit                 = local.express_route_circuit_name
    express_route_peering                 = local.express_route_peering_name
    express_route_peering_connection      = local.express_route_peering_connection_name
    express_route_connection              = local.express_route_connection_name
    log_analytics_cluster                 = local.log_analytics_cluster_name
    log_analytics_workspace               = local.log_analytics_workspace_name
    ampls                                 = local.ampls_name
    private_endpoint_resource             = local.private_endpoint_resource_name
    azure_dashboard                       = local.azure_dashboard_name
  }
}
