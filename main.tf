locals {
  ...
  network_security_group_rule_name           = "${var.direction}-${var.action}-${var.source_resource}-${var.destination_resource}-${var.direction}"
  ...
  private_endpoint_resource_name             = "per-${var.workload}-${var.environment}-${var.resource_name}-${var.vnet_name}" # A new implementation for private endpoint resource name, use when more specific names are required
  storage_account_name                       = "st${var.storage_description}${substr(var.environment, 0, 3)}${substr(md5("st${var.storage_description}"), 0, 6)}"
  ...
  express_route_name                         = "er-${var.workload}-${var.environment}"
  express_route_circuit_name                 = "erc-${var.workload}-${var.environment}"
  express_route_peering_name                 = "erp-${var.erp_type}-${var.er_circuit_name}-${var.environment}"
  express_route_connection_name              = "erc-${var.workload}-${var.environment}"
  log_analytics_cluster_name                 = "la-cluster-${var.environment}"
  express_route_peering_connection_name      = "erpc-${var.erp_name}-${var.er_gateway_name}"
  log_analytics_workspace_name               = "law-${var.law_purpose}-${var.environment}"
  ampls_name                                 = "ampls-${var.workload}-${var.environment}"
  azure_dashboard_name                       = "ad-${var.workload}-workbook"
